...
setwd("S:/NicheNet_esercitazione/Data")
...
## Sender: MAIT cells 
#we want to select MAIT cells at timepoint P3. For B cells it was already done by tutors
Idents(MAIT) = 'time'
expressed_genes_sender = get_expressed_genes("P3", MAIT, pct = 0.10) #it selects all genes expressed by MAIT at timepoint 3, at least by 10% of the cells.
expressed_genes_sender <- expressed_genes_sender[expressed_genes_sender %in% colnames(ligand_target_matrix)] #select only those that are present in the list of ligands in the matrix
...
B_P3_c2 = subset(B_P3, idents = '2') #the idents is already set

Idents(B_P3_c2) = 'time'
expressed_genes_receiver =  get_expressed_genes("P3", B_P3_c2, pct = 0.10)
background_expressed_genes =  expressed_genes_receiver[expressed_genes_receiver %in% rownames(ligand_target_matrix)] #only those present among target genes
...
### Define a gene set of interest
  ...
  #DEG between P3 and P0 in cluster 2 of B cells
...
rownames(DEG_filter) = DEG_filter$names #names of DEG, to use as a list of genes of interest
geneset_oi = rownames(DEG_filter)
head(geneset_oi)
length(geneset_oi)

## Among the DEGs, we have to select only the one in the ligand_target_matrix
...
expressed_ligands = intersect(ligands, expressed_genes_sender) #select ligands expressed by MAIT cells
expressed_receptors =  intersect(receptors, expressed_genes_receiver) #select receptors expressed by B cells in cluster 2

potential_ligands =  lr_network %>% filter(from %in% expressed_ligands & to %in% expressed_receptors) %>% pull(from) %>% unique() #select from the matrix all ligands that have an intereaction with the receptors present in MAIT cels
...
#this finds the ligand with a strong interaction, computing the pearson coefficient, which represent the strenght of the interaction
...
#the weight is not based on my analysis, but it comes from data online and dataset.
